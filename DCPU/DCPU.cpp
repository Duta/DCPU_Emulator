...
void DCPU::step()
{
	DCPU_WORD instruction = NEXT_WORD;
	if(shouldExecute)
	{
...
{
	shouldExecute = true;
	A = B = C = X = Y = Z = I = J = PC = EX = IA = 0;
	SP = RAM_SIZE - 1;
	resetRAM();
	clearHardwareConnections();
}

void DCPU::connectHardware(Hardware *newHardware)
{
	for(unsigned int i = 0; i < MAX_CONNECTED_HARDWARE; ++i)
	{
		if(hardware[i] == nullptr)
		{
			hardware[i] = newHardware;
			return;
		}
	}
	...
	std::cerr << newHardware->toString() << std::endl;
}
...
void DCPU::executeInstruction(const DCPU_WORD instruction)
{
	...
	case 0x02:
		// ADD b, a
		*bp += *ap;
		// TODO: If overflow, set EX to 0x0001, else 0x0
		break;
	case 0x03:
		// SUB b, a
		*bp -= *ap;
		// TODO: If underflow, sets EX to 0xFFFF, else 0x0
		break;
	case 0x04:
		// MUL b, a
		*bp *= *ap;
	...
	case 0x06:
		// DIV v, a
		*bp /= *ap;
		// TODO: Set EX to ((b<<16)/a)&0xFFFF
		//       If a==0, set b and EX to 0 instead. (Treats b, a as unsigned)
		break;
	case 0x07:
		// DVI b, a
		*bp = (DCPU_WORD_S)*bp / (DCPU_WORD_S)*ap;
	...
	case 0x1B:
		// SBX b, a
		*bp -= *ap - EX;
		// TODO: If underflow, set EX to 0xFFFF, else 0x0
		break;
	case 0x1E:
		// STI b, a
		*bp = *ap;
...
DCPU_WORD * DCPU::getValue(const DCPU_WORD val, bool a)
{
	...
	case 0x10:
		return &RAM[A + NEXT_WORD];
	case 0x11:
		return &RAM[B + NEXT_WORD];
	case 0x12:
		return &RAM[C + NEXT_WORD];
	case 0x13:
		return &RAM[X + NEXT_WORD];
	case 0x14:
		return &RAM[Y + NEXT_WORD];
	case 0x15:
		return &RAM[Z + NEXT_WORD];
	case 0x16:
		return &RAM[I + NEXT_WORD];
	case 0x17:
		return &RAM[J + NEXT_WORD];
